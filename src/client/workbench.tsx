...
@media(max-width:980px){.np-shell{inset:0}.np-main{grid-template-columns:1fr}.np-side{display:none}.np-timebar{grid-template-columns:120px 1fr}.np-time-value{display:none}.np-picker{width:calc(100vw - 20px);height:calc(100vh - 20px)}}
`
...
function WorkspaceFilePicker({ controller, cancellable, onCancel }: {
  ...
  useEffect(() => setSelected(undefined), [listing?.path])

  const cancel = () => { if (onCancel === undefined) controller.hide(); else onCancel() }
  const enter = (entry: NeuroWorkspaceEntry) => {
    if (entry.type === 'file') {
      setSelected(entry)
      return
    }
    if (listing === undefined) return
    setHistory(current => [...current, listing.path])
    void controller.browse(listing.workspace.id, entry.path)
  }
  const back = () => {
    if (listing === undefined || history.length === 0) return
    const target = history.at(-1)
    setHistory(current => current.slice(0, -1))
    if (target !== undefined) void controller.browse(listing.workspace.id, target)
  }

  return (
    <div className="np-dialog-backdrop">
      <section className="np-dialog np-picker" role="dialog" aria-modal="true" aria-label="Choose neuroscience data from workspace">
        <header className="np-picker-head">
          <div className="np-picker-title">
            <div><h2>Open from workspace</h2><div className="np-picker-sub">Choose a NIfTI-1 <code>.nii</code> file one folder at a time.</div></div>
            {cancellable ? <button className="np-button" onClick={cancel}>Cancel</button> : null}
          </div>
          {listing === undefined ? null : <div className="np-picker-path" title={listing.path}>{listing.path}</div>}
        ...
        <div className="np-picker-body">
          {listing === undefined ? (
            workspaces.length === 0
              ...
              : <div className="np-workspaces">{workspaces.map(workspace => <button className="np-picker-row np-workspace-row" key={workspace.id} onClick={() => { setHistory([]); void controller.browse(workspace.id) }}><span className="np-picker-mark">▣</span><span className="np-picker-name">{workspace.title}<span className="np-picker-detail">{workspace.path}</span></span></button>)}</div>
          ) : listing.entries.length === 0
            ? <div className="np-picker-empty"><div><strong>No supported files here</strong>Open another folder or go back. Only <code>.nii</code> is shown.</div></div>
            : listing.entries.map(entry => <button
              className={`np-picker-row${selected?.path === entry.path ? ' np-picker-row-selected' : ''}`}
              key={`${entry.type}:${entry.path}`}
              onClick={() => enter(entry)}
              onDoubleClick={() => { if (entry.type === 'file') void controller.open(entry.path) }}
            ><span className="np-picker-mark">{entry.type === 'directory' ? '▸' : 'N'}</span><span className="np-picker-name">{entry.name}</span><span className="np-picker-size">{entry.type === 'directory' ? 'Folder' : formatBytes(entry.size)}</span></button>)}
        </div>
        <footer className="np-picker-foot">
          <div className="np-picker-foot-main">
            <div>{listing !== undefined && workspaces.length > 1
              ? <button className="np-link-button" onClick={() => { setHistory([]); controller.showWorkspaceRoots() }}>Change workspace</button>
            ...
            <div className="np-picker-foot-actions">{listing !== undefined && history.length > 0 ? <button className="np-button" onClick={back}>Back</button> : null}<button className="np-button np-button-primary" disabled={snapshot.loading || selected === undefined} onClick={() => { if (selected !== undefined) void controller.open(selected.path) }}>{snapshot.loading ? 'Loading…' : 'Open viewer'}</button></div>
          </div>
