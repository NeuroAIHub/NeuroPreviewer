...
import type { NeuroWorkspaceEntry, SignalInteractiveDataset, SignalPreviewView, WireImage2DFrame, VoxelCursor, WireInteractivePreviewView } from '../core/types.js'
...
.np-picker-anchor{position:fixed;z-index:90;left:292px;top:72px;pointer-events:none}.np-picker-anchor .np-picker{pointer-events:auto}.np-picker{width:min(590px,calc(100vw - 320px));height:min(560px,calc(100vh - 100px));padding:0;box-shadow:0 18px 48px #0008}.np-picker-body{padding:8px}.np-tree-row{min-height:38px;padding-top:4px;padding-bottom:4px}.np-tree-chevron{width:18px;height:24px;border:0;background:transparent;color:#9ba2a6;padding:0;cursor:pointer}.np-tree-chevron:hover{color:#eef0f1}.np-tree-root{border-bottom:1px solid #303538;margin-bottom:4px}.np-tree-loading{color:#9ba2a6;padding:7px 10px;font-size:12px}.np-logo{width:20px;height:20px;display:block;color:#42d3c8;flex:none}.np-brand{display:flex;align-items:center;gap:9px}.np-brand-copy{min-width:0}.np-trigger{box-sizing:border-box;border:1px solid transparent;border-radius:10px;font-weight:500}.np-trigger-wide{position:relative;min-width:0;width:100%;height:38px;justify-content:flex-start;padding:8px 16px;gap:0;font-size:14px;line-height:22px}.np-trigger-wide .np-logo{position:absolute;left:calc(var(--np-native-icon-x,50%) - 8px);width:14px;height:14px}.np-trigger-wide span{position:absolute;left:calc(var(--np-native-icon-x,50%) + 12px);top:8px;margin:0}.np-trigger-compact{min-width:36px;width:36px;height:36px;justify-content:center;padding:0;gap:0}.np-trigger-compact .np-logo{width:18px;height:18px}.np-trigger:hover{border-color:var(--dsw-alias-border-l2,#ffffff18)}[data-neuro-previewer-top-entry]{flex:none;margin:0 2px 8px}[data-neuro-previewer-top-entry]:has(.np-trigger-compact){width:36px;align-self:flex-start;margin:0 0 12px}[data-neuro-previewer-top-entry] .np-trigger{color:var(--dsw-alias-label-primary,var(--ds-text-primary,#eee))}[data-neuro-previewer-top-entry] .np-trigger:hover{background:var(--dsw-alias-interactive-bg-hover,#ffffff0c)}
.np-signal-main{min-height:0;display:grid;grid-template-columns:minmax(0,1fr) 280px}.np-signal-stack{min-height:0;overflow:hidden;padding:10px;background:#090b0d;display:grid;gap:5px}.np-signal-row{min-height:0;position:relative;border:1px solid #282d31;background:#030404}.np-signal-row svg{display:block;width:100%;height:100%}.np-signal-line{fill:none;stroke:#5d8ff0;stroke-width:1;vector-effect:non-scaling-stroke}.np-signal-zero{stroke:#ffffff20;stroke-width:1;vector-effect:non-scaling-stroke}.np-signal-label{position:absolute;left:7px;top:5px;z-index:1;color:#8fb2ff;background:#000b;padding:2px 5px;font:10px ui-monospace,SFMono-Regular,monospace}.np-signal-range{position:absolute;right:7px;top:5px;color:#9ba2a6;background:#000b;padding:2px 5px;font:9px ui-monospace,SFMono-Regular,monospace}.np-signal-controls{display:grid;gap:14px}.np-signal-controls select{width:100%;padding:7px;background:var(--dsw-specific-input-major,#fff);color:var(--dsw-alias-label-primary,#0f1115);border:1px solid var(--dsw-alias-border-l2,#0000001a);border-radius:8px}
...
function signalPoints(trace: SignalPreviewView['traces'][number]): string {
  const range = trace.max > trace.min ? trace.max - trace.min : 1
  const last = Math.max(1, trace.samples.length - 1)
  return trace.samples.map((value, index) => `${(index / last * 1000).toFixed(2)},${(92 - (value - trace.min) / range * 84).toFixed(2)}`).join(' ')
}

function SignalWorkbench({ controller, dataset, view, loading }: {
  readonly controller: NeuroViewerController
  readonly dataset: SignalInteractiveDataset
  readonly view: SignalPreviewView
  readonly loading: boolean
}) {
  const [startSample, setStartSample] = useState(view.startSample)
  const [windowSamples, setWindowSamples] = useState(view.windowSamples)
  const [channelStart, setChannelStart] = useState(view.traces[0]?.channel ?? 0)
  const [showPicker, setShowPicker] = useState(false)
  const visibleChannels = Math.min(8, dataset.metadata.channelCount)
  const maxStart = Math.max(0, dataset.metadata.sampleCount - windowSamples)
  useEffect(() => {
    setStartSample(view.startSample)
    setWindowSamples(view.windowSamples)
    setChannelStart(view.traces[0]?.channel ?? 0)
  }, [dataset.datasetId])
  useEffect(() => {
    const timeout = window.setTimeout(() => void controller.view({
      startSample: Math.min(startSample, maxStart), windowSamples,
      channelStart, channelCount: visibleChannels,
    }), 80)
    return () => window.clearTimeout(timeout)
  }, [channelStart, controller, dataset.datasetId, maxStart, startSample, visibleChannels, windowSamples])
  const seconds = windowSamples / dataset.metadata.sampleRate
  const windowChoices = [1, 5, 10, 30, 60].map(value => Math.min(dataset.metadata.sampleCount, Math.max(1, Math.round(value * dataset.metadata.sampleRate)))).filter((value, index, all) => all.indexOf(value) === index)
  return <>
    <style>{workbenchCss}</style>
    <section className="np-shell" role="dialog" aria-modal="true" aria-label="NeuroPreviewer signal viewer">
      <header className="np-top">
        <div className="np-brand"><NeuroPreviewerLogo /><span className="np-brand-copy">NeuroPreviewer<small>SIGNAL WORKBENCH</small></span></div>
        <div className="np-path" title={dataset.path}>{dataset.path}</div>
        <div className="np-actions"><button className="np-button" onClick={() => { setStartSample(0); setChannelStart(0) }}>Reset</button><button className="np-button np-button-primary" onClick={() => { setShowPicker(true); void controller.loadWorkspaces() }}>Open file</button><button className="np-button np-button-danger" onClick={() => controller.hide()} aria-label="Close viewer">Close</button></div>
      </header>
      <div className="np-signal-main">
        <main className="np-signal-stack" style={{ gridTemplateRows: `repeat(${Math.max(1, view.traces.length)},minmax(0,1fr))` }}>
          {view.traces.map(trace => <div className="np-signal-row" key={trace.channel}>
            <span className="np-signal-label">{trace.label} · {trace.unit || 'a.u.'}</span><span className="np-signal-range">{trace.min.toPrecision(4)} … {trace.max.toPrecision(4)}</span>
            <svg viewBox="0 0 1000 100" preserveAspectRatio="none" aria-label={`${trace.label} waveform`}><line className="np-signal-zero" x1="0" x2="1000" y1="50" y2="50" /><polyline className="np-signal-line" points={signalPoints(trace)} /></svg>
          </div>)}
        </main>
        <aside className="np-side np-signal-controls">
          <section className="np-section"><div className="np-section-title">Signal window</div><div className="np-control"><label>Window <output>{seconds.toFixed(1)} s</output></label><select aria-label="Signal window duration" value={windowSamples} onChange={event => { const next = Number(event.target.value); setWindowSamples(next); setStartSample(current => Math.min(current, Math.max(0, dataset.metadata.sampleCount - next))) }}>{windowChoices.map(value => <option value={value} key={value}>{(value / dataset.metadata.sampleRate).toFixed(value < dataset.metadata.sampleRate * 10 ? 1 : 0)} seconds</option>)}</select></div><div className="np-control"><label>First channel <output>{channelStart + 1} / {dataset.metadata.channelCount}</output></label><input aria-label="First signal channel" type="range" min="0" max={Math.max(0, dataset.metadata.channelCount - visibleChannels)} value={channelStart} onChange={event => setChannelStart(Number(event.target.value))} /></div></section>
          <section className="np-section"><div className="np-section-title">Dataset</div><div className="np-kv"><span>Format</span><b>{dataset.metadata.format.toUpperCase()}</b><span>Channels</span><b>{dataset.metadata.channelCount}</b><span>Sample rate</span><b>{dataset.metadata.sampleRate.toFixed(2)} Hz</b><span>Samples</span><b>{dataset.metadata.sampleCount}</b><span>Duration</span><b>{dataset.metadata.durationSeconds.toFixed(2)} s</b></div></section>
          {dataset.warnings.map(warning => <div className="np-warning" key={warning}>{warning}</div>)}
        </aside>
      </div>
      <footer className="np-timebar" style={{ gridTemplateColumns: '120px minmax(0,1fr) 170px' }}><div className="np-time-value" style={{ textAlign: 'left' }}><strong>{view.timeStart.toFixed(2)} s</strong>window start</div><div className="np-time"><label><span>TIME</span><output>{view.timeStart.toFixed(2)} — {view.timeEnd.toFixed(2)} s</output></label><input aria-label="Signal time position" type="range" min="0" max={maxStart} value={Math.min(startSample, maxStart)} onChange={event => setStartSample(Number(event.target.value))} /></div><div className="np-time-value"><strong>{view.timeEnd.toFixed(2)} s</strong>{seconds.toFixed(1)} s window</div></footer>
      {loading ? <div className="np-loading">Updating preview…</div> : null}
      ...
      <section className="np-dialog np-picker" role="dialog" aria-modal="false" aria-label="Choose neuroscience data from workspace">
        <header className="np-picker-head">
          <div className="np-picker-title">
            <div><h2>Open from workspace</h2><div className="np-picker-sub">Choose NIfTI, EDF/EDF+, BrainVision, EEGLAB, or NWB data.</div></div>
        ...
        <div className="np-picker-body">
          {listing === undefined ? (
            ...
            {expanded.has(listing.path) ? listing.entries.length === 0
              ? <div className="np-picker-empty"><div><strong>No supported files here</strong>Expand another folder to find volume or signal data.</div></div>
...
export function NeuroWorkbench({ controller }: { readonly controller: NeuroViewerController }) {
  ...
  const dataset = snapshot.dataset?.kind === 'volume' ? snapshot.dataset : undefined
  const view = snapshot.view !== undefined && !('kind' in snapshot.view) ? snapshot.view : undefined
  const signalView = snapshot.view !== undefined && 'kind' in snapshot.view && snapshot.view.kind === 'signals' ? snapshot.view : undefined
  ...
  if (snapshot.dataset?.kind === 'signals' && signalView !== undefined) return <SignalWorkbench controller={controller} dataset={snapshot.dataset} view={signalView} loading={snapshot.loading} />
...
function NeuroPreviewTrigger({ wide, controller }: { readonly wide: boolean, readonly controller: NeuroViewerController }) {
  return <><style>{workbenchCss}</style><button className={`np-trigger ${wide ? 'np-trigger-wide' : 'np-trigger-compact'}`} onClick={() => controller.show()} title="Open NeuroPreviewer"><NeuroPreviewerLogo />{wide ? <span>NeuroPreviewer</span> : null}</button></>
}
...
export function NeuroPreviewSidebarEntry({ wide, controller }: { readonly wide: boolean, readonly controller: NeuroViewerController }) {
  ...
  useEffect(() => {
    ...
    const alignToNativeIcon = () => {
      const icon = newSession.querySelector('svg')
      if (icon === null) return
      const buttonBounds = newSession.getBoundingClientRect()
      const iconBounds = icon.getBoundingClientRect()
      target.style.setProperty('--np-native-icon-x', `${iconBounds.left + iconBounds.width / 2 - buttonBounds.left}px`)
    }
    const observer = new ResizeObserver(alignToNativeIcon)
    observer.observe(newSession)
    alignToNativeIcon()
    setHost(target)
    return () => { observer.disconnect(); target.remove() }
  }, [])
